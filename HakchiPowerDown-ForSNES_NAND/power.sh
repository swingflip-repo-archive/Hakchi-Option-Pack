...
echo "                             $(date '+%d %b %Y  @ %H:%M') \#No internal clock. What a shit show!" >> /var/tmp/Hakchi_terminal.log
echo "$(tput setaf 2)======================================================================================" >> /var/tmp/Hakchi_terminal.log	   
...
#if [ "frig" == "frig" ]
#then
#	if /bin/retroarch-mini null &> /var/tmp/RetroArch_test_runtime.log ; then
#		echo "RetroArch successfully ran using the test rig at $(date)" >> /var/tmp/Hakchi_terminal.log
#	else
#		echo "RetroArch FAILED to run using the test rig at $(date)" >> /var/tmp/Hakchi_terminal.log
#	fi
#fi
gameover &> /var/tmp/Hakchi_terminal.log
#/media/fbpad/fbpad &> /var/tmp/FBpad_runtime.log

#If install == install run YAFT and do a benchmark analysis
#if [ "install" == "no" ]
#then
#	echo "RUNNING INSTALL OF YAFT" >> /var/tmp/Hakchi_terminal.log
#	echo "$(/bin/mkdir -p /usr/share/terminfo)" &> /var/tmp/Yaft_runtime.log
#	echo "$(/bin/tic -o /usr/share/terminfo /media/yaft-0.2.9/info/yaft.src)" &> /var/tmp/Yaft_runtime.log
#	echo "$(/bin/mkdir -p /usr/bin/)" &> /var/tmp/Yaft_runtime.log
#	echo "$(/bin/install -m755 /media/yaft-0.2.9/yaft /usr/bin/yaft)" &> /var/tmp/Yaft_runtime.log
#	echo "$(/bin/install -m755 /media/yaft-0.2.9/yaft_wall /usr/bin/yaft_wall)" &> /var/tmp/Yaft_runtime.log
#	echo "$(/bin/mkdir -p /usr/share/man/man1/)" &> /var/tmp/Yaft_runtime.log
#	echo "$(/bin/install -m644 /media/yaft-0.2.9/man/yaft.1 /usr/share/man/man1/yaft.1)" &> /var/tmp/Yaft_runtime.log
#	echo "FINISHED INSTALL OF YAFT" >> /var/tmp/Hakchi_terminal.log
#	#echo "$(/media/yaft-0.2.9/yaft)" &> /var/tmp/Yaft_runtime.log
#	if /media/yaft-0.2.9/yaft &> /var/tmp/Yaft_runtime.log ; then
#		echo "YAFT successfully ran YAFT at $(date)" >> /var/tmp/Hakchi_terminal.log
#	else
#		echo "YAFT FAILED to run YAFT at $(date)" >> /var/tmp/Hakchi_terminal.log
#	fi
#fi